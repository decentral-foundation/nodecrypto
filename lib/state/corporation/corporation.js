...
class Corporation {
  ...
  static CADENCE = ["MOM","YOY"];
  static SHARE_TYPES = ["COMMON","PREFERRED"];
  ...
  /**
   * @param {string} - Share type
   */
  issueShares(amount,shareType) {
    this.authorized_shares -= amount;
    this.issued_shares += amount;
  }

  /**
   * @param {number} dividendPerShare - const value of dividend pay out in dollars
   */
  enableDividends(dividendsPerShare) {
    if (this.authorized_shares === 0 || typeof this.authorized_shares == "undefined") {
      throw new Error("Cannot create dividends without shares issued");
    }
    this.dividends_per_share = dividendsPerShare;
  }
  ...
  franchiseTaxAuthorizedSharesMethod(state) {
    ...
    if (authorized_shares > 10000) {
      let shares_over = authorized_shares - 10000; //shares less ten thousand dollars
  ...
  /**
   * @description - calculate potential scenario using EPS formula
   * @param {number} - amount in dollars
   * @param {dividendPayRatio} - float with 3 point precision
   */
  calculateEarningsPerShare(targetNetIncome, dividendPayRatio) {
    let preferredDiv = this.total_shares * dividendPayRatio;
    console.log("this.authorized_shares: ",this.authorized_shares);
    let commonSharesArray = range(0,this.authorized_shares);
    console.log("commonSharesArray: ",commonSharesArray.length);
    let commonShares = commonSharesArray.filter(share => {
      return true; // this code to be updated once common vs preferred has been impl
    }).length;
    console.log("commonShares: ",commonShares);
    console.log("commonShares: ",commonShares);
    return (targetNetIncome - preferredDiv)/commonShares;

    // confirmed equation is 
  ...
  getNetIncome() {
    // confirmed equation is 
    // (avg common stock holder equity) * 
    // (return on common stockholder equity) + 
    // (preferred dividends)
  }

  getAverageCommonStockHoldersEquity() {

  }

  *iterator() {
    let index = 0;
    let methodNames = Object.getOwnPropertyNames(this.__proto__);
    for (let yIndex of Object.getOwnPropertyNames(this.__proto__)) {
      yield yIndex;      
    }
  }
